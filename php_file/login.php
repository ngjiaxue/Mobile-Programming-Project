...
if ($resultadmin->num_rows > 0) {
    while ($row = $resultadmin ->fetch_assoc()){
        echo "success admin&".$row["NAME"]."&".$row["EMAIL"]."&".$row["PHONE"];
    }
}else if($resultuser->num_rows > 0){
    while ($row = $resultuser ->fetch_assoc()){
        echo "success&".$row["NAME"]."&".$row["EMAIL"]."&".$row["PHONE"]."&".$row["ADDRESS"]."&".$row["LATITUDE"]."&".$row["LONGITUDE"]."&".$row["CREDIT"];
    }
}else if($resultuserwrongpasswordonly->num_rows > 0){
    echo "incorrect password";
}else if($resultnoverify->num_rows > 0){
